...
/**
 * Load queue length options from database
 */
...
/**
 * Build radio button queue options in the report form
 */
function buildQueueLengthOptions() {
    ...
    queueLengthOptions.forEach(function (feature, index) {
        ...
        wrapper.innerHTML =
            ...
            props.queue_length_description +
            '</label>';
...
/**
 * Open the cleanliness/queue report form for a hospital
 *
 * @param {Object} properties - Hospital feature properties
 */
function openQueueCleanlinessForm(properties) {
    document.getElementById("report_hospital_id").value = properties.hospital_id || "";
    document.getElementById("report_hospital_name").value = properties.hospital_name || "";
    document.getElementById("report_user_id").value = userId || "";
    document.getElementById("report_hospital_name_display").innerText = properties.hospital_name || "";
...
/**
 * Opens queue form from popup lookup by hospital id
 *
 * @param {string|number} hospitalId - hospital id key
 */
 ...
 * Save new hospital
 *
 * @param {Object} formValues - values from hospital form
 */
function saveNewHospital(formValues) {
    ...
    fetch(apiBase + '/api/crudAPI/insertHospital', {
        ...
        .then(function (data) {
            ...
            // keep a visible marker immediately
            ...
            document.getElementById("hospitalFormDialog").close();
 ...
 *
 * @param {Object} formValues - values from report form
 */
function saveQueueCleanlinessReport(formValues) {
    ...
    fetch(apiBase + '/api/crudAPI/insertCleanlinessQueueReport', {
        ...
        .then(function (data) {
            document.getElementById("queueCleanlinessFormDialog").close();
            document.getElementById("queueCleanlinessForm").reset();
...
/**
 * Load user hospitals from the API and show them on the default layer
 */
...
/**
 * Get number of cleanliness/queue reports for the current user
 */
...
/**
 * Get the five closest hospitals to the user's current location
 * and show them as a reporting layer.
 */
...
/**
 * Get hospitals with unknown queue length for this user
 * and show them as a reporting layer.
 */
...
/**
 * Get hospital bar chart data.
 * For now this logs the data until chart rendering is added.
 */
